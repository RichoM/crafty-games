...
(function (parent) {
	...
	*/
		search: function (rect, filter) {
			/*
			var currentFrame = Crafty.frame();
			if (currentFrame !== lastFrame) {
				console.log(cache);
				cache = {};
				lastFrame = currentFrame;
			}
			var kkkk = rect._entityName || "Richo";
			if (cache[kkkk] === undefined)
				cache[kkkk] = 1;
			else
				cache[kkkk] = cache[kkkk] + 1;
			*/
			// ..............
			var results,
				currentFrame = Crafty.frame();
			
			if (rect.__cache === undefined ||
				rect.__cache.frame !== currentFrame) {
...
Crafty.c("Collision", {
	...
	*/
	hit: function (comp) {
		var area = this._mbr || this,
			results = Crafty.map.search(this, false),
