...
window.onload = (function() {	
	...
	Crafty.scene("Main", function () {
		map = Crafty.e("2D, Canvas, Draggable, TiledMapBuilder").createWorld(SOURCE_FROM_TILED_MAP_EDITOR, function (tiledMap) {
			tiledMap.attr({w: 1300, h: 1300});			
			...
			Crafty.c("Car", {
				...
				step: function () {
					...
					if (this.speed != 0) {
						if (left) {
							this.steeringWheel -= this.turnSpeed;
						} else if (right) {
							this.steeringWheel += this.turnSpeed;
						}
					}
					
					if ((left && this.steeringWheel > 0)
						|| (right && this.steeringWheel < 0)
						|| !(left || right)) {
						this.steeringWheel = 0;
					}
					if (Math.abs(this.steeringWheel) > this.maxTurnSpeed) {
						this.steeringWheel = this.maxTurnSpeed * (this.steeringWheel > 0 ? 1 : -1);
					}
					...
					// Turn
					this.rotation += this.speed * this.steeringWheel / 10;
				},
