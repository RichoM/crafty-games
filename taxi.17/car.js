Crafty.c("Car", {
	...
	init: function () {		
		this.addComponent("2D, Canvas, Bounds, Collision, Tween, MarkFollower")
			.attr({w: 33, h: 55})
			.origin(16, 40)
			...
			.onHit("Start", function () {/* Do nothing */}, this.noHitStart);			
	},
	...
	noHitStart: function () {
		if (this.currentMark === undefined) return;
		if (this.markCount === this.currentMark.count()) {
			this.lap++;
			this.resetMarkCount();
